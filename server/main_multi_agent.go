...
func chatWithLLM(c *gin.Context) {
	...
	payload := map[string]interface{}{
		...
		"databricks_options": map[string]interface{}{"return_trace": true},
	}
	...
	if len(llmResp) == 0 || len(llmResp[0].Choices) == 0 || llmResp[0].Choices[0].Message.Content == "" {
	...
	content := llmResp[0].Choices[0].Message.Content
